...
// Values array
// values will start with a capital letter when dealing with json files
type Values struct {
...
func printTable(r []reading) {
	...
	for _, v := range r {
		total := v.normal + v.outofrange
		percent := v.outofrange / total * 100
		fmt.Printf("%v\t%v\t%v\t%5v\t\t%5.1f\n", v.hour, total, v.normal, v.outofrange, percent)
	}
}
...
func printChart(r []reading) {
	...
	for _, v := range r {
		msg := fmt.Sprintf("Hour %d", v.hour)
		bar := chart.StackedBar{
			Name: msg,
			Values: []chart.Value{
				{Value: float64(v.normal), Label: "Green"},
				{Value: float64(v.outofrange), Label: "Red"},
			},
		}
